...
function StudentMarkets() {
  ...
  return (
    <section className="flex-grow bg-white px-4 xl:px-10 pt-6">
      {!isPostAdd && (
        <>
          ...
          <div className="grid grid-cols-1 gap-6 mt-4">
            {categories.map((category) => {
              const content = (
                <div
                  key={category.id}
                  className="relative flex overflow-hidden w-full h-30 sm:h-50 shadow-lg hover:shadow-md transition-shadow cursor-pointer"
                  onClick={() => {
                ...
                >
                  <div className="w-1/3 xl:w-1/5 h-full overflow-hidden">
                    <img
                      src={category.image}
                      alt={category.name}
              ...
              return category.hasSubcategories ? content : content;
            })}
          </div>
