...
function ProfileSettings() {
    ...
    const settingsItems = [
        {id:1,name:'Personal Details',route:`/personal-details/${user.role}`},
  ...
  return (
    <section className="w-full pl-3 sm:ml-5 max-w-sm mx-auto mt-10 sm:max-w-xl md:ml-4 md:max-w-2xl bg-gray-50 p-4 rounded-md shadow-sm">
        ...
        <div className='flex flex-col w-full mt-8'>
            ...
            <span className='w-full flex items-center justify-between p-2 hover:bg-gray-100 cursor-pointer' key={item.id}
            onClick={()=>{
                if (item.route) {
                    navigate(item.route);
                }
                ...
                {item.name === 'Notification' ? (
                    ...
                    user.isVerified ? (
                        ...
                        <span className=" text-xs sm:text-sm text-red-500">Not Verified</span>
                    )
                    ) : (
        ...
        <span className='w-full flex items-center justify-between pt-2 pb-2 mt-2 hover:bg-gray-100 cursor-pointer'
