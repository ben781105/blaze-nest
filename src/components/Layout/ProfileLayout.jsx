    ...
    <section className="bg-white   pb-4 flex flex-col gap-12 ml-4 mr-4 border border-gray-200 shadow-md ">
      <div className="relative flex items-center justify-center z-40 bg-gradient-to-b from-blue-400 via-blue-300 to-amber-100 h-30 w-full ">
       <div className="flex text-center md:place-self-center place-self-start justify-start flex-col">
        {user.isVerified? <h1 className="text-white md:text-xl">Account Verified!</h1>:
        <h1 className="text-white text-sm  md:text-xl">Account Not Verified</h1>}
      ...
      <div className={`mt-10 w-full space-y-5 ${user.isVerified?'opacity-100':'opacity-50'}`}>
        <h1>My Listings</h1>
            ...
            <div key={listing.id} className="flex w-full md:gap-6  h-fit  ">
             ...
             <div className="flex flex-col md:flex-row gap-3 md:gap-6 pt-3 pl-3 w-1/2">
              <span className="space-y-1">
                ...
                <p>{listing.status}</p>
                <p>{listing.price}</p>
                <p className="mt-4 text-gray-400"> Posted {listing.posted}</p>
              </span>
             <div className="flex gap-3 ">
              <button className=" border border-blue-500 h-9 w-15 rounded-lg">Edit</button>
              <button className="border border-red-500 p-1 h-9 w-22 rounded-lg">Remove</button>
             </div>
             </div>
            </div>
